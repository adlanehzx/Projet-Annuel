<template>
  <div style="max-width:1200px;margin:0 auto;padding:24px 24px 48px;width:100%">
    <h1 style="font-family:var(--font-display);font-weight:700;font-size:26px;letter-spacing:-0.01em;margin:0 0 14px;color:var(--text-primary)">Recommandations pour vous</h1>
    <p style="font-size:14px;color:var(--text-secondary);margin:0 0 24px;max-width:580px">Basé sur vos animés vus, vos notes et vos listes</p>
    ...
    <div v-else-if="recommendations.length === 0" style="text-align:center;padding:64px;color:var(--text-secondary)">
      Revenez bientôt ! Notez au moins un animé 7/10 ou plus pour débloquer des recommandations personnalisées basées sur vos goûts.
    </div>
    ...
    <div v-else style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:20px">
      ...
      >
        ...
        <div style="margin-top:3px;font-family:var(--font-mono);font-size:12px;color:var(--rating)">★ {{ item.score?.toFixed(1) ?? '—' }}</div>
      </div>
    </div>
  </div>
</template>
