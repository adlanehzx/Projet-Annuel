  ...
  <div style="max-width:1000px;margin:0 auto;padding:24px 24px 48px;width:100%">
    ...
    <template v-else>
      <div style="background:var(--bg-elevated);border:1px solid var(--border);border-radius:14px;padding:24px;display:flex;gap:20px;align-items:center;flex-wrap:wrap">
        <div style="width:72px;height:72px;border-radius:50%;background:var(--color-accent-secondary);color:#fff;display:flex;align-items:center;justify-content:center;font-family:var(--font-display);font-weight:700;font-size:24px;letter-spacing:-0.01em;flex-shrink:0">
        ...
        <div style="flex:1;min-width:200px">
          ...
          <div v-if="profile.bio" style="font-size:14px;line-height:1.55;color:var(--text-primary);margin-top:8px;white-space:pre-wrap">{{ profile.bio }}</div>
          <div style="font-size:13px;color:var(--text-secondary);margin-top:6px">Membre depuis {{ joinDate }}</div>
        </div>
      </div>
      ...
      <div v-if="publicLists.length > 0" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:16px">
        ...
        >
          ...
          <div style="font-family:var(--font-mono);font-size:12px;color:var(--text-secondary);margin-top:6px">{{ (list.animes || []).length }} animé{{ (list.animes || []).length !== 1 ? 's' : '' }}</div>
        </div>
      </div>
