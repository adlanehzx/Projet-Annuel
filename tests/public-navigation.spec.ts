...
test.describe('Parcours public', () => {
  test('la page d’accueil présente AnimeTrack et les actions principales', async ({ page }) => {
    await page.goto('/');

    await expect(page).toHaveTitle(/AnimeTrack/);
    await expect(page.getByRole('heading', { name: /votre carnet d'anime/i })).toBeVisible();
  ...
  test('un visiteur peut accéder au catalogue et revenir à la connexion', async ({ page }) => {
    ...
    await expect(page.getByPlaceholder('Rechercher un anime…')).toBeVisible();

    await page.getByRole('link', { name: 'Connexion' }).click();
    await expect(page).toHaveURL(/\/auth\/login$/);
    await expect(page.getByRole('heading', { name: /connexion à animetrack/i })).toBeVisible();
  });
});
